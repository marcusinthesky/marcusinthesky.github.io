...
import { PCA_POINT_COUNT, PCA_ROTATION_DEGREES, pcaPoints, pcaSummary } from "./pca-cloud-model";
import styles from "./pca-cloud.module.css";
...
export function PcaCloud() {
  ...
  return (
    ...
    >
      ...
      <g
        className={`${styles.frame} ${styles.rotate}`}
        style={
          {
      ...
      >
        ...
        <g className="fill-none stroke-foreground" strokeWidth="1.25">
          {axes.flatMap(({ name, paths }, index) =>
            paths.map((d) => (
              <path
                className={index === 0 ? `${styles.draw} stroke-data-estimate` : styles.draw}
                d={d}
                key={`${name}-${d}`}
                pathLength={1}
                strokeDasharray="1"
        ...
        >
          {axes.map(({ label, name, ratio }, index) => (
            ...
            <g
              className={`${styles.label} ${styles.counterRotate}`}
              key={name}
