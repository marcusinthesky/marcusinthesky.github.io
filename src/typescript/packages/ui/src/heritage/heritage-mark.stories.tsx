...
import { HeritageMark, heritageMotifs } from "./heritage-mark";
import { InstitutionalDevice } from "./institutional-device";
import { Motif, type MotifTone } from "./motif";
import { OpenBook, RouteDots, Rook, TableMountain } from "./parts/heritage";
import { Anchor, AntiqueLamp, MottoRibbon, MuralCrown } from "./parts/institutions";
import { CompassStar } from "./parts/patterns";
import { Lotus, MottoScroll, PalmBranch, PrayerHands, Rose, Shuttle, Torse } from "./parts/prain";
...
/**
 * One drawing is not merely scaled. A mark (24px and below) keeps the silhouette
 * and main lines; the default adds washes and ribs; a study adds hatching and veins.
 */
export const MarkVsStudy: Story = {
  render: () => (
    <table className="border-collapse text-sm">
      <thead>
        <tr>
          ...
          {(["mark", "default", "study"] as const).map((detail) => (
            <th className="p-2" key={detail} scope="col">
              {detail}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {(
          ...
          <tr className="border-t border-border" key={name}>
            ...
            <td className="p-2">
              <Motif className="size-6" detail="mark" viewBox="0 0 256 256">
                {part}
              </Motif>
            </td>
            <td className="p-2">
              <Motif className="size-40 bg-card" viewBox="0 0 256 256">
                {part}
              </Motif>
            </td>
            <td className="p-2">
              <Motif className="size-40 bg-card" detail="study" viewBox="0 0 256 256">
                {part}
              </Motif>
            </td>
          </tr>
